...
export default async function TiendaPage({ searchParams }: Props) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans pb-24">
      ...
      <div className="bg-white border-b border-slate-200 pt-6 pb-6 mb-8">
        <div className="max-w-7xl mx-auto px-8">
          <ScrollReveal as="slide-right">
            <h1 className="text-2xl md:text-3xl font-extrabold text-[#0F172A] tracking-tight mb-1.5">
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          ...
          <div className="w-full lg:w-3/4">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 pb-4 border-b border-slate-200">
              <h2 className="text-lg font-bold text-[#0F172A]">Resultados</h2>
              
              <div className="flex items-center gap-3 w-full sm:w-auto">
                <StoreSortSelect />
                
                <div className="hidden sm:flex text-sm font-medium text-slate-500 bg-white px-4 py-2 rounded-lg shadow-sm border border-slate-200 items-center gap-2">
              ...
              {filteredProducts.length > 0 ? (
                filteredProducts.map((product: Product, index: number) => (
                  <ScrollReveal key={product.id} as="fade-up" delay={(index % 3) * 80}>
                    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-xl hover:shadow-orange-900/5 hover:border-orange-200 transition-all duration-500 flex flex-col justify-between group h-full hover:-translate-y-1">
                      ...
                      <div className="relative aspect-square w-full bg-white overflow-hidden border-b border-slate-50">
                        ...
                        <div className="absolute top-3 left-3 z-20 bg-white/90 backdrop-blur-md text-[#0F172A] shadow-sm border border-slate-100 text-[10px] font-bold px-3 py-1.5 rounded-full uppercase tracking-wider group-hover:-translate-y-0.5 transition-transform duration-300">
                          {product.category || 'Equipo'}
                        </div>

                        {product.brand && (
                          <div className="absolute top-3 right-3 z-20 bg-orange-50/90 backdrop-blur-md text-orange-700 shadow-sm border border-orange-100 text-[10px] font-bold px-3 py-1.5 rounded-full uppercase tracking-wider group-hover:-translate-y-0.5 transition-transform duration-300">
                            {product.brand}
                          </div>
                        )}
                      </div>
                      ...
                      <div className="p-5 flex-grow flex flex-col">
                        <Link href={`/products/${product.id}`} className="block group/link mb-2">
                          <h3 className="text-base font-bold text-[#0F172A] group-hover/link:text-orange-600 transition-colors line-clamp-2 leading-tight">
                            {product.name}
                          </h3>
                        </Link>
                      ...
                      <div className="p-5 pt-4 bg-slate-50/50 border-t border-slate-100 flex items-center justify-between group-hover:bg-orange-50/30 transition-colors duration-300">
                        <div>
                          <span className="text-[10px] text-slate-400 block font-semibold uppercase tracking-wider mb-0.5">IVA Incluido</span>
                ...
                <div className="col-span-full py-20 text-center bg-white rounded-2xl border border-dashed border-slate-300 flex flex-col items-center justify-center">
                  <div className="w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center mb-5 text-slate-400">
                    <PackageSearch className="w-8 h-8" />
                  </div>
                  <h3 className="text-xl font-extrabold text-[#0F172A] mb-2">No encontramos equipos</h3>
                  <p className="text-slate-500 text-sm max-w-sm mx-auto mb-6">
                    Ajusta los filtros de precio, marca o categoría para ver más resultados.
                  </p>
                  <Link href="/tienda" className="px-6 py-2.5 bg-orange-600 text-white font-semibold rounded-xl hover:bg-orange-700 transition-colors shadow-sm">
